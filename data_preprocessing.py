...
def make_sample_by_feature(adult, larva):
    # Load the data, *without* setting the index yet
    adult = pd.read_csv(adult, sep='\t', header=None, skipinitialspace=True)
    larva = pd.read_csv(larva, sep='\t', header=None, skipinitialspace=True)
